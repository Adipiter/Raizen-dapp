        ...
        setTotalVault(
          (
            await contract.getBalance(
              "0x563911820C498496b250A5b756feE74338C0A5F8"
            )
          ).toString()
        );
      ...
      if (chainId != process.env.NEXT_PUBLIC_CHAIN_ID) {
        ...
        if (
          parseFloat(
            conv.convertWei(
              String(
                (((sec - totalStake.time_stake) / 3600) *
                  totalStake.total_amount) /
                  1000
              ),
              "ether"
            )
          ...
          try {
            ...
            const gasLimit = await contract.estimateGas.withdrawStake(
              totalStake.total_amount,
              0
            );
            const gasPrice = await web3Provider.getGasPrice();

            const staking = await contract.withdrawStake(
              totalStake.total_amount,
              0,
              {
                gasLimit: gasLimit,
    ...
    if (!active) {
      ...
      if (chainId != process.env.NEXT_PUBLIC_CHAIN_ID) {
        ...
        if (totalBalance < 100) {
          ...
          if (
            parseFloat(
              conv.convertWei(String(totalStake.total_amount), "ether")
            ).toFixed(2) > 100
          ) {
            return toast({
              title: "Stake Exists",
            ...
            try {
              ...
              const gasLimit = await contract.estimateGas.stake(
                String(
                  conv.convertEther(
                    (80 / 100) * conv.convertWei(totalBalance, "ether"),
                    "wei"
                  )
                )
              );
              ...
              const withdrawing = await contract.stake(
                String(
                  conv.convertEther(
                    (80 / 100) * conv.convertWei(totalBalance, "ether"),
                    "wei"
          ...
          <GridItem h="auto">
            ...
            >
              ...
              <Text fontSize="lg" color="#fff" fontWeight="bold">
                {active
                  ? parseFloat(
                      conv.convertWei(totalBalance.toString(), "ether")
                    ).toFixed(0)
                  : "0.00"}{" "}
                {process.env.NEXT_PUBLIC_MAIN_CURRENCY}
              </Text>
            </Flex>
          </GridItem>
              ...
              <Flex>
                ...
                >
                  <Text>
                    {active
                      ? parseFloat(
                          conv.convertWei(String(totalVault), "ether")
                        ).toFixed(0)
                      : "0.00"}{" "}
                    {process.env.NEXT_PUBLIC_MAIN_CURRENCY}
                  </Text>
              ...
              <Flex>
                ...
                >
                  <Text>
                    {active
                      ? parseFloat(
                          conv.convertWei(String(totalCollab), "ether")
                        ).toFixed(0)
                      : "0.00"}{" "}
                    {process.env.NEXT_PUBLIC_BRIDGE_CURRENCY}
                  </Text>
                </StatNumber>
              </Flex>
            </Stat>
            ...
            <ModalBody>
              <List spacing={3}>
                ...
                <ListItem>
                  ...
                  Raizzen tokens are only exclusively valid for use within the
                  Raizzen's ecosystem , and hence do not have any real-world
                  value and shall not be considered as real-world currency.
                </ListItem>
                <ListItem>
                  <ListIcon as={FiDatabase} color="green.500" />
                  You can only place a stake once, and you must first withdraw
                  your existing stake in order to place another one.
                </ListItem>
                {/* You can also use custom icons from react-icons */}
                <ListItem>
                  <ListIcon as={FiRefreshCw} color="green.500" />
                  Reward per hour will fluctuate dependent on the current demand
                  for this project.
                </ListItem>
              </List>
            </ModalBody>
